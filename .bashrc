...
git_color() {
    ...
    if [[ "$STATUS" == *'Not a git repository'* ]]
        ...
        if [[ "$STATUS" != *'working tree clean'* ]]
            then echo -e '\033[1;91m' # red if need to commit
        else
            if [[ "$STATUS" == *'Your branch is ahead'* ]]
                then echo -e '\033[1;93m' # yellow if need to push
            else
                echo -e '\033[1;92m' # else green
            fi
        fi
    fi
}
...
export PS1=$BWhite'\u'$BWhite' → '$BYellow'[\w]\e[0m$(git_color)$(git_branch)\n'$White'\$ '
...
# See top 10 resource heavy programs runnning right now
...
# Show open ports
## Install netcat and net-tools for this
